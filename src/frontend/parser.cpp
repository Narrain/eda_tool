...
int Parser::getBinOpPrecedence(const std::string &op) {
    if (op == "||") return 1;
    if (op == "&&") return 2;
    if (op == "==" || op == "!=") return 3;
...
BinaryOp Parser::mapBinaryOp(const std::string &op) {
...
DataType Parser::parseDataType(const std::string &kw) {
    DataType dt;
...
std::unique_ptr<Expression> Parser::parseBinaryRHS(int exprPrec, std::unique_ptr<Expression> lhs) {
    while (true) {
        if (peek().kind != TokenKind::Symbol) return std::move(lhs);
